...
def encode(secret_access_key, str_to_encode, urlencode=False):
    ...
    b64_hmac = base64.encodestring(hmac.new(secret_access_key.encode('utf-8'), str_to_encode.encode('utf-8'), sha).digest()).strip()
    if urlencode:
...
def add_auth_header(access_key_id, secret_access_key, headers, method, bucket, key, query_args):
    ...
    encode_sk = encode(secret_access_key, c_string)
    if isinstance(encode_sk, bytes):
        encode_sk = encode_sk.decode('utf-8')
    authorization = "%s %s:%s" % ("KSS", access_key_id, encode_sk)
